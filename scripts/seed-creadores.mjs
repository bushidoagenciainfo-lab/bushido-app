/**
 * Importa creadores al book desde una lista curada.
 * Uso (desde bushido-app, con las env de Supabase cargadas):
 *   node --env-file=.env.local scripts/seed-creadores.mjs
 * Sin Supabase configurado, escribe en .data/creadores.json (pruebas locales).
 *
 * Es idempotente por (nombre + ciudad): si el creador ya está, no lo duplica.
 */
...
// ── Cúcuta · base inicial ──────────────────────────────────────────────
// nichos/formatos quedan vacíos a propósito: se clasifican desde /admin
// cuando sepamos qué hace cada quien (mejor vacío que mal etiquetado).
const CUCUTA = [
...
async function main() {
  if (!URL || !KEY) {
    ...
    const file = path.join(dir, "creadores.json");
    let all = [];
    try {
    ...
    for (const c of TODOS) {
      if (all.some((x) => x.nombre === c.nombre && x.ciudad === c.ciudad)) continue;
      all.push({ id: crypto.randomUUID(), ...c, estado: "nuevo", created_at: new Date().toISOString() });
      nuevos++;
    }
    await fs.writeFile(file, JSON.stringify(all, null, 2), "utf8");
    console.log(`(local) ${nuevos} creadores nuevos · total ${all.length} → ${file}`);
    return;
  }

  const db = createClient(URL, KEY, { auth: { persistSession: false } });
  const { data: existentes, error: e1 } = await db.from("creadores").select("nombre, ciudad");
  if (e1) throw new Error(e1.message);
  const yaEsta = new Set((existentes ?? []).map((r) => `${r.nombre}|${r.ciudad}`));

  const faltan = TODOS.filter((c) => !yaEsta.has(`${c.nombre}|${c.ciudad}`));
  if (!faltan.length) {
    console.log("Nada que importar: ya están todos.");
    return;
  }
  ...
  console.log(`✓ Importados ${faltan.length} creadores (${TODOS.length - faltan.length} ya estaban).`);
}
